...
#Potential energy and forces with the Anisotropic cubic potential           
def localenergy(xs,ys,zs,rs,i,nlist):
    ...
    for j in nlist[i]:
        if j != i:
            ...
            r = (rs[i] + rs[j]) - d 
            if r>0:
                U += 0.333 * ((rs[i]+rs[j])/(rs[i]*rs[j])) * k * r**3
    return U
...
def force_residual(xs,ys,zs,rs,i,nlist): #returns the force components on i'th droplet
    ...
    for j in nlist[i]:
        if j!=i:
            ...
            if r>0:
                fx += -k *  ((rs[i]+rs[j])/(rs[i]*rs[j])) * r**2 * (xs[j]-xs[i])/d
                fy += -k *  ((rs[i]+rs[j])/(rs[i]*rs[j])) * r**2 * (ys[j]-ys[i])/d
                fz += -k *  ((rs[i]+rs[j])/(rs[i]*rs[j])) * r**2 * (zs[j]-zs[i])/d
    
    return fx ,fy, fz

#### Taylor expanded actual potential -> Doesnt work :( ###
#def localenergy(xs,ys,zs,rs,i,nlist):
#    U = 0
#    for j in nlist[i]:
#        if j != i:
...
#            x = (rs[i] + rs[j]) - d 
#            if x>0:
#                U +=  ( ( (rs[i]+rs[j]) * (x**4 - 4*(rs[i]+rs[j])*x**3 + 12*(rs[i]*rs[j])*x**2 + 3*((rs[i]-rs[j])**2)*(rs[i]+rs[j])*x - 3*rs[i]**4 - 3*rs[j]**4 + 6*(rs[i]**2)*(rs[j]**2)   ) ) / ( (12*rs[i]*rs[j]) * (rs[i] + rs[j]) ) ) * (1 + (x/(rs[i]+rs[j])) + (x/(rs[i]+rs[j]))**2 + (x/(rs[i]+rs[j]))**3 + (x/(rs[i]+rs[j]))**4 )                     
#
#    return U
#
#def totalenergy(xs,ys,zs,rs,nlist):
...
#        if j!=i:
#            d = ((((xs[i]-xs[j])**2 + (ys[i]-ys[j])**2 + (zs[i]-zs[j])**2)**0.5)) #Distance bw centers 
#            x = (rs[i] + rs[j]) - d  #Overlap 
#            if x>0:
#                fx += - (( (rs[i]+rs[j])*(2*rs[i] + 2*rs[j] - x)*(x)*(2*rs[i]-x)*(2*rs[j]-x) ) / ( (rs[i]*rs[j])*(4)*((rs[i] + rs[j])**2 ) )) * (1 + (x/(rs[i]+rs[j]))**2 + (x/(rs[i]+rs[j]))**4 ) * ((xs[j]-xs[i])/d)
#                fy += - (( (rs[i]+rs[j])*(2*rs[i] + 2*rs[j] - x)*(x)*(2*rs[i]-x)*(2*rs[j]-x) ) / ( (rs[i]*rs[j])*(4)*((rs[i] + rs[j])**2 ) )) * (1 + (x/(rs[i]+rs[j]))**2 + (x/(rs[i]+rs[j]))**4 ) * ((ys[j]-ys[i])/d)
#                fz += - (( (rs[i]+rs[j])*(2*rs[i] + 2*rs[j] - x)*(x)*(2*rs[i]-x)*(2*rs[j]-x) ) / ( (rs[i]*rs[j])*(4)*((rs[i] + rs[j])**2 ) )) * (1 + (x/(rs[i]+rs[j]))**2 + (x/(rs[i]+rs[j]))**4 ) * ((zs[j]-zs[i])/d)
#    
#    return fx ,fy, fz
